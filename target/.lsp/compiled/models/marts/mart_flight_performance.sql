...
from `flight-analytics-dbt`.`intermediate_models`.`int_flight_metrics` f
left join `flight-analytics-dbt`.`core_models`.`dim_airline` a
    on f.operating_airline_id = a.airline_id
left join `flight-analytics-dbt`.`core_models`.`dim_routes` r
