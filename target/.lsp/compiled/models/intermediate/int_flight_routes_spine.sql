...
left join `flight-analytics-dbt`.`staging_models`.`stg_routes` r
    on fd.flight_id = r.flight_id
left join `flight-analytics-dbt`.`staging_models`.`stg_departures` dep
    on fd.flight_id = dep.flight_id
left join `flight-analytics-dbt`.`staging_models`.`stg_arrivals` arr
    on fd.flight_id = arr.flight_id
left join `flight-analytics-dbt`.`intermediate_models`.`int_airport_geography` dep_geo
    on fd.departure_airport_id = dep_geo.airport_id
left join `flight-analytics-dbt`.`intermediate_models`.`int_airport_geography` arr_geo
