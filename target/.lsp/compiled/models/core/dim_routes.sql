...
from `flight-analytics-dbt`.`intermediate_models`.`int_routes_spine` r
left join `flight-analytics-dbt`.`core_models`.`dim_airport` dep
    on r.departure_airport_id = dep.airport_id
left join `flight-analytics-dbt`.`core_models`.`dim_airport` arr
